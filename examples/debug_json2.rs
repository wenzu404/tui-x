...
async fn main() -> Result<()> {
    ...
    // Dump the full first tweet user_results
    ...
    {
        if let Some(result) = entries.first()
            .and_then(|e| e.pointer("/content/itemContent/tweet_results/result"))
        {
            let user_result = result.pointer("/core/user_results/result").unwrap();
            // Print all top-level keys of user result
            println!("=== User result top-level keys:");
            if let Some(obj) = user_result.as_object() {
                for key in obj.keys() {
                    println!("  {key}");
                }
            }
            // Print full user JSON (truncated)
            let s = serde_json::to_string_pretty(user_result)?;
            println!("\n=== Full user JSON (first 3000 chars):\n{}", &s[..s.len().min(3000)]);
        }
    }
